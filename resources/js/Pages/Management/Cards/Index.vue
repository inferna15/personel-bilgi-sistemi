...
// Prop tiplerini, HttpService'den gelen JSON yapısına uygun olarak tanımlıyoruz.
interface CardData {
    id: number;
    user_id: number;
    full_name: string;
...
interface Props {
    data: { // Bu, Laravel'in paginator çıktısının tamamını içeren ana 'data' prop'u
        current_page: number;
        data: CardData[]; // Asıl kart verileri burada
...
watch(search, debounce((value) => {
    router.get(route('management.cards.index'), { search: value }, {
        preserveState: true,
        replace: true,
    });
}, 300));
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Kart Yönetimi</h2>
        </template>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 bg-white border-b border-gray-200">
                        <div class="flex flex-col md:flex-row justify-between items-center mb-6">
                            <h3 class="text-2xl font-bold text-gray-800">Kart Listesi</h3>
                            <div class="flex items-center space-x-4 mt-4 md:mt-0">
                                ...
                                />
                                <Link :href="route('management.cards.create')" class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded-md transition duration-300">
                                    Yeni Kart Ekle
                                </Link>
                            </div>
                        </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Veriliş Tarihi</th>
                                    <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">İşlemler</th>
                                </tr>
                                </thead>
                                <tbody class="bg-white divide-y divide-gray-200">
                                <tr v-if="props.data.data.length === 0"> <!-- props.data.data olarak değişti -->
                                    <td colspan="7" class="px-6 py-4 whitespace-nowrap text-center text-sm text-gray-500">
                                        Hiç kart bulunamadı.
                                    </td>
                                </tr>
                                <tr v-for="card in props.data.data" :key="card.id"> <!-- props.data.data olarak değişti -->
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <Link :href="route('management.cards.show', card.id)"
                                        ...
                                        <Link method="delete" :href="route('management.cards.destroy', card.id)"
                        ...
                        <div v-if="props.data.links.length > 3" class="mt-8 flex justify-center">
                            <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                                <!-- Previous Button -->
                                <Link
                                    :href="props.data.prev_page_url || '#'" :class="[ // props.data.prev_page_url olarak değişti
                                        'relative inline-flex items-center px-2 py-2 rounded-l-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50',
                                        !props.data.prev_page_url ? 'opacity-50 cursor-not-allowed' : '' ]"
                                :disabled="!props.data.prev_page_url" > // props.data.prev_page_url olarak değişti
                                ...
                                <!-- Next Button -->
                                <Link
                                    :href="props.data.next_page_url || '#'" :class="[ // props.data.next_page_url olarak değişti
                        ...
                        <div class="mt-4 text-center text-sm text-gray-600">
                            {{ props.data.from || 0 }}-{{ props.data.to || 0 }} arası gösteriliyor, toplam {{ props.data.total }} kart.
                        </div>
                    </div>
                </div>
            </div>
